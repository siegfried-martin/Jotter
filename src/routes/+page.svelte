...
<script lang="ts">
  ...
  onMount(async () => {
    // Only check for redirect if this is NOT an intentional navigation
    // (i.e., user didn't navigate here from within the app)
    const isIntentionalNavigation = $navigating !== null || 
                                   document.referrer.includes('/app/collections/') ||
                                   $page.url.searchParams.has('no-redirect');
    
    if (!isIntentionalNavigation && !hasCheckedRedirect) {
      console.log('📱 Initial app load, checking for redirect...');
      ...
      console.log('📱 Intentional navigation to collections page, skipping redirect');
    }
