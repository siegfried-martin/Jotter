...
import { DemoSequenceService } from '@/lib/services/localStorage/demoStorageService';
...
export async function getNextCollectionSequence(userId: string): Promise<number> {
  ...
  const { data, error } = await supabase.rpc('get_next_collection_sequence', { p_user_id: userId });
...
export async function getNextNoteContainerSequence(collectionId: string): Promise<number> {
  ...
  const { data, error } = await supabase.rpc('get_next_note_container_sequence', {
...
export async function getNextNoteSectionSequence(noteContainerId: string): Promise<number> {
  ...
  const { data, error } = await supabase.rpc('get_next_note_section_sequence', {
    p_note_container_id: noteContainerId
  });
...
export async function updateCollectionSequences(updates: SequenceUpdate[]): Promise<boolean> {
  ...
  try {
    const promises = updates.map((update) =>
      supabase.from('collections').update({ sequence: update.sequence }).eq('id', update.id)
    );
...
export async function updateNoteContainerSequences(updates: SequenceUpdate[]): Promise<boolean> {
  ...
  try {
    const promises = updates.map((update) =>
      supabase.from('note_container').update({ sequence: update.sequence }).eq('id', update.id)
    );
    ...
    if (hasErrors) {
      console.error(
        'Some note container sequence updates failed:',
        results.filter((r) => r.error)
      );
      return false;
    }
...
export async function updateNoteSectionSequences(updates: SequenceUpdate[]): Promise<boolean> {
  ...
  try {
    const promises = updates.map((update) =>
      supabase.from('note_section').update({ sequence: update.sequence }).eq('id', update.id)
    );
    ...
    if (hasErrors) {
      console.error(
        'Some note section sequence updates failed:',
        results.filter((r) => r.error)
      );
      return false;
    }
